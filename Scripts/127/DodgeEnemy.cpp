...
void DodgeEnemy::Init(device* pd3dDevice)
{
...
void DodgeEnemy::Update()
{
	if (m_vPos.x <= 0) m_vVelocity.x *= -1;
	if (m_vPos.x >= 1600 - 128) m_vVelocity.x *= -1;
	if (m_vPos.y <= 0) m_vVelocity.y *= -1;
	if (m_vPos.y >= 900 - 51) m_vVelocity.y *= -1;
	...
	if (++RandCnt > 100)
	{
		m_vPos.x = (rand() % 1600) -200;
		m_vPos.y = (rand() % 900) - 100;
		RandCnt = 0;
	}
...
void DodgeEnemy::Render(sprite* pd3dSprite)
{
	pd3dSprite->Draw(
		m_Tex
