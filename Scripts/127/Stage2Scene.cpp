...
void Stage2Scene::Init(device* pd3dDevice)
{
	pBG = new Stage2BG();
	pBG->Init(pd3dDevice);

	pTile = new Stage2Tile();
	pTile->Init(pd3dDevice);

	pPlayer = new Stage2Player();
	...
	pBoss = new Stage2Boss();
	pBoss->Init(pd3dDevice);

	pInfected = new Stage2InfectedBG();
...
void Stage2Scene::Update()
{
	...
	if (m_bProof)
	{
		vector3 vTemp = vector3(pPlayer->m_vPos.x - 90, pPlayer->m_vPos.y - 120, 0);
	...
	pInfected->m_bCell[iy - 1][ix + 1] = false;
	pInfected->m_bCell[iy + 1][ix + 1] = false;
	pInfected->m_bCell[iy][ix - 1] = false;
	pInfected->m_bCell[iy - 1][ix - 1] = false;
	pInfected->m_bCell[iy + 1][ix - 1] = false;
	...
	pInfected->m_bCell[iy - 1][ix] = false;
	pInfected->m_bCell[iy][ix + 2] = false;
	pInfected->m_bCell[iy][ix - 2] = false;
	pInfected->m_bCell[iy + 2][ix] = false;
	pInfected->m_bCell[iy - 2][ix] = false;
	...
	RECT rtPlayer = { pPlayer->m_vPos.x, pPlayer->m_vPos.y, pPlayer->m_vPos.x + 64, pPlayer->m_vPos.y + 24 };
	...
	m_nScore = ((float)nF / ((H2 - 1) * W2)) * 100.0f;
	m_nDisScore = ((float)inF / ((H2 - 1) * W2)) * 100.0f;
...
void Stage2Scene::Render(sprite* pd3dSprite)
{
